...
final class PAGFileImpl extends PAGFile with PAGCompositionImpl {
  ...
  PAGFileImpl.asset(String asset)
    : api = rootBundle.load(asset).then((data) async {
        final pag = await js.libPAG;
        return pag.PAGFile.load(data.buffer.toJS).toDart;
      }),
      super.impl();

  /// Web 端无本地文件路径概念，将 file 视为 URL 用 fetch 加载。
  PAGFileImpl.file(String file)
    : api = js.libPAG.then((pag) async {
        final response = await js.fetch(file).toDart;
        if (!response.ok) {
          throw StateError('Failed to load $file: HTTP ${response.status}');
        }
        final buffer = await response.arrayBuffer().toDart;
        return pag.PAGFile.load(buffer).toDart;
      }),
      super.impl();
  ...
  PAGFileImpl.bytes(Uint8List bytes)
    : api = js.libPAG.then((pag) => pag.PAGFile.load(bytes.buffer.toJS).toDart),
      super.impl();
}
